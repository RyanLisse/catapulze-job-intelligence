...
| `NEXT_PUBLIC_SERVER_URL` | `https://api.catapulze.com` (tijdelijk: `https://api.23-88-60-222.sslip.io`) |
| `INTERNAL_SERVER_URL` | interne URL van de `server`-application in Coolify (`http://<server-servicenaam>:3000`); sinds #124 gebruikt de Next.js-server die voor de Better Auth-sessiecheck |
| `TRIGGER_SECRET_KEY` | `Job Intelligence local/TRIGGER_SECRET_KEY` (RJC-373 blijft open) |

Opgelost in #124 (`INTERNAL_SERVER_URL`); een deploy van vóór d4ef1e8 rendert `/dashboard` in de container nog met een SSR-fout.
...
4. **Deploy per runbook stap 4–9** — loopt op 2026-09-03 als Codex-lane op **tijdelijke sslip.io-hostnamen** (`api.23-88-60-222.sslip.io`, `app.23-88-60-222.sslip.io`) zodat de echte DNS pas na bewijs omgaat (manticore 6.3.8, redis, migrator → `bun run db:migrate` moet 15/15 rapporteren zonder wijzigingen, server, web, projector met `apps/server/Dockerfile.projector`). Alle secrets via Coolify's secret-UI uit §2/§3. Bewijs: `/version` = merge-SHA, `/readyz` ready op elk component, `searchProjection` ok met lag 0.
...
- Gemerged: #122 (registerV1Id faalt gesloten bij re-point van `v1_id`) en #124 (`INTERNAL_SERVER_URL`).
...
- `onefellow.spec.ts` was datumafhankelijk; main (#124) fixt het met een toekomstige deadline. Andere fixtures met echte deadlines blijven een risico.
