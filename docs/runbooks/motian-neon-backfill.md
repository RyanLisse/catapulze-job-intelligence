...
| Legacy commercial fields (CTP-515/527/528/529/530) | `provincie` comes only from the Motian `province` column or a province named in the vacancy title (Werkzoeken publishes it there), both through `normalise/provincie.ts`; a city in `location` never yields a province. `skills` collects published lists from the `competences`, `requirements` and `wishes` JSONB columns (entries are strings or objects with a `name` member) and is shaped by the shared `normaliseSkills`; it stays absent when no list is published. `eind_datum` also reads the typed `end_date` column, and an unparsable `application_deadline` yields no `sluitingsdatum` instead of an invalid instant. No new nested `raw_payload` key is read: on 2026-09-15 no `MOTIAN_DATABASE_URL` credential existed on the build VM, so nested spellings for hours, start, deadline and description could not be proven against a real payload and were removed rather than shipped inert. Only 323bddf's proven `rateMin`/`rateMax`/`educationLevel` nested lifts remain. |
