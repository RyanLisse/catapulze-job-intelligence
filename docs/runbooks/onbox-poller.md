...
Per source live flags (`TENDER_NED_LIVE`, `INHUURDESK_LIVE`, and the rest) work
exactly as they did under Trigger: `process.env[source.liveEnv] === "1"` decides
whether a connector talks to the real site or to fixtures.
...
The poller has no HTTP surface, so liveness is a file. Every cycle writes the
current epoch millis to `POLLER_HEARTBEAT_FILE`; the Dockerfile HEALTHCHECK runs
`bun src/poller/heartbeat.ts --check` and reports healthy while that file is
younger than 60 seconds. That entrypoint is a separate module from `main.ts` on
purpose, so a health check every 15 seconds does not boot the typed environment,
the database client and the connector registry.
...
- **`poller_cycle`**: one line per cycle with `pollable`, `due` and `durationMs`.
...
| SIGINT / SIGTERM | Aborts the loop. The source in flight always finishes first, including its backlog drain, so a poll is never killed mid-write. Then a shutdown line, the lock release, the connection close, and exit 0. A repeated signal is logged as `poller_shutdown_in_progress` and otherwise ignored. SIGKILL remains the only hard stop and can leave the advisory lock held until Postgres notices the dead connection. |
