...
| `geen` then term or list | `geen zzp`, `geen zzp mogelijk`, `geen zzp'ers gezocht`, `geen freelancers.`, `geen zzp of freelance`, `geen zzp of detachering` |
| labelled answer | `zzp: nee`, `zzp mogelijkheid: nee`, `zzp toegestaan: nee` |
| not intended for | `niet voor zzp`, `niet bedoeld voor freelancers`, `deze opdracht staat niet open voor zzp'ers` |

Two shapes are deliberately left out, and two more are known misses.

The `geen` row accepts a coordinated list, and the list may name other contract
forms: `geen zzp of detachering` excludes both, so neither may come back as the
answer. `geen ZZP of detachering, alleen vast dienstverband` therefore reports
`vast`, taking the form stated outside the excluded list. A comma list needs no
rule of its own, because the clause splitter already cuts on commas.
