...
| L3f JSON-LD-cohort: Randstad, Techniekwerkt | Gedeelde `packages/connectors/src/json-ld/` adapter; de twee bronfiles en configs zijn ongewijzigd — de lane voegde alleen bewijs toe | **Fixture-bewijs op het duurzame pad**; operator-canary en releasegate blijven open, dus geen live-runtimeclaim | CTP-642: `json-ld/durable-cohort-l3f.spec.ts` (14 specs — incl. `detailSynthesizer`-pad én fail-closed voor Techniekwerkt's `vike_pageContext`) + `poller/json-ld-cohort-l3f.integration.spec.ts` (10 specs op echte `ji_test_iso_*`-Postgres) + geseedde-stack UI-bewijs; rollout één bron tegelijk via `POLLER_DURABLE_BRONNEN` |
