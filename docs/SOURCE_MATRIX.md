...
| **ProUnity** (HeadFirst Group, BE; CTP-543) | `pro-unity.com/sitemap.xml` → `pji_job-sitemap*.xml` → SSR `/job/<uuid>/` (geen JobPosting JSON-LD) | titel, referentie (K…), werkperiode (start+eind), land (Belgium), rollen/talen/skills-tags, beschrijving, apply-URL; géén tarief/deadline/opdrachtgever-veld | robots `Crawl-delay: 10` + WooCommerce-disallows; ToS-IP/databankclausule → `te_toetsen` (BE-scope GO gegeven) | ±4.800 URL's incl. historie; connector `prounity` (html-adapter); zie `sources/prounity.md` |
...
| **ProUnity** (CTP-543; HeadFirst Group, BE) | **BUILD** (BE-scope GO + `te_toetsen` op ToS-IP/databankclausule) | `pji_job-sitemap*.xml` (±4.800 `/job/<uuid>/` incl. historie), listing 45 missies 100 % BE (allemaal in nieuwste chunk, geverifieerd 2026-09-18), `Crawl-delay: 10`, ToS verbiedt "copy, analyze … content encumbered with Intellectual Property Rights" | Connector `prounity` gebouwd; zie rung-2-rij en `sources/prounity.md` |
