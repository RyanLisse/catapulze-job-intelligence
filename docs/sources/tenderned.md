...
**End-to-end op de echte pipeline** (fixture-client, `ji_test_iso_*`-Postgres): offer → `runDurableBronJobConsumer` → `runBronIngestPipeline` → observaties, `source_record`s en curated `aanvraag`-rijen; gefaalde run → herval op hetzelfde `scrapeRunId` (`reopenFailed`, fence +1) → resume vanaf checkpoint; abort mid-item → run `failed` (persistence-abort is nooit "benign", CTP-490) → retake leest de resterende pagina's alsnog — geen verlies, exact-één. Bewijs: `feed-cohort.integration.spec.ts` (6 specs, groen), bewaard als voorgestelde patch onder `/private/tmp/w8-evidence/feed-cohort/` omdat `apps/worker` buiten deze lane valt.
