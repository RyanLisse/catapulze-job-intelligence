...
/**
 * CTP-641 L3e JSON-LD-cohort proof: ProRail, Rijkswaterstaat, TBI, TenMonks
 * and Jobbird on the durable ingest path (`curated.durable_job` +
 * `runDurableBronJobConsumer` + `runBronIngestPipeline`) against real
 * Postgres — the committed fixture clients for detail reads, scripted
 * whole-corpus listings for retake/abort evidence.
 *
 * All five connectors share the single-page discovery contract at the
 * connector level: `discover()` returns every detail URL with
 * `hasMore: false` and an inert `checkpoint: {}` — including the two
 * non-sitemap members. ProRail's `json-listing` page walking lives inside
 * `client.fetchListing` (`discovery.pagination` fans out page 2..N before
 * the connector sees the list), and Jobbird's `listing` deliberately covers
 * page one only. A durable retake therefore re-enumerates the WHOLE corpus
 ...
 * by page-cursor resume. These specs assert that honestly.
 *
 * Corpus scoping, stated plainly: the committed listing fixtures are real
 * recordings (prorail 16 API hits on one page, rijkswaterstaat 5 sitemap
 * `<loc>`s → 3 vacancy URLs after exclusion, tbi 5 `<loc>`s → 2 vacancy
 * URLs, tenmonks 3 `<loc>`s, jobbird 15 freelance detail links), while
 * detail coverage is the fixture-backed subset recorded per bron (1–3
 * URLs). `scopedFixtureConnector` therefore filters the REAL parsed listing
 * to `config.detailFixtures` keys — every persisted payload is a real
 * recorded fixture, and whole-corpus enumeration itself is pinned per bron
 * in `packages/connectors/src/json-ld/durable-cohort-l3e.spec.ts` and the
 * per-source listing specs.
 * ProRail does carry a recorded soft-404 detail fixture
 * (`detail-sollicitatie-soft-404.json`), but its URL is no longer in the
 * recorded listing — the scoped corpus cannot reach it, so
 * `rejectedReferenties` stays empty here and the reject path is covered by
 * the connector's per-source specs.
 */
...
const previousEnv = {
  ...
  JOBBIRD_LIVE: process.env.JOBBIRD_LIVE,
  PRORAIL_LIVE: process.env.PRORAIL_LIVE,
  RAW_OBJECT_STORE_PATH: process.env.RAW_OBJECT_STORE_PATH,
  RIJKSWATERSTAAT_LIVE: process.env.RIJKSWATERSTAAT_LIVE,
  SEARCH_PROJECTOR: process.env.SEARCH_PROJECTOR,
  TBI_LIVE: process.env.TBI_LIVE,
  TENMONKS_LIVE: process.env.TENMONKS_LIVE,
};
...
process.env.RAW_OBJECT_STORE_PATH ??= `${process.env.TMPDIR ?? "/tmp"}/ji-jsonld-l3e-cohort-raw-${process.pid}`;
// All five connectors must stay in fixture mode — no live egress from tests.
delete process.env.JOBBIRD_LIVE;
delete process.env.PRORAIL_LIVE;
delete process.env.RIJKSWATERSTAAT_LIVE;
delete process.env.TBI_LIVE;
delete process.env.TENMONKS_LIVE;
...
interface BronSpec {
   ...
   * tbi and tenmonks have a single fixture-backed URL (nothing can persist
   * first — the retake writes it fresh).
   */
...
const PRORAIL: BronSpec = {
  ...
  discoveredReferenties: [
    "vacatures/functie/woordvoerder",
    "vacatures/verkeersleiding/treinverkeersleider-maastricht",
  ],
  fixtureReferenties: [
    "vacatures/functie/woordvoerder",
    "vacatures/verkeersleiding/treinverkeersleider-maastricht",
...
const RIJKSWATERSTAAT: BronSpec = {
  abortOnDetailCall: 2,
  bronId: "00000000-0000-4000-8000-00000000001b",
  bronSlug: "rijkswaterstaat",
  categorie: "werkgever",
  config: rijkswaterstaatConfig,
  discoveredReferenties: [
    "vacatures/adviseur-assetmanagement-rivierbodem/1330716",
    "vacatures/adviseur-waterveiligheid/1310882",
    "vacatures/jurist-handhaving/1318820",
  ],
  fixtureReferenties: [
    "vacatures/adviseur-assetmanagement-rivierbodem/1330716",
    "vacatures/adviseur-waterveiligheid/1310882",
...
const TBI: BronSpec = {
  ...
  // corpus (same shape as the L3b werken-voor-nederland case).
  abortOnDetailCall: 1,
  bronId: "00000000-0000-4000-8000-000000000011",
  bronSlug: "tbi",
  categorie: "werkgever",
  config: tbiConfig,
  discoveredReferenties: ["vacatures/service-technicus-w-1280611"],
  fixtureReferenties: ["vacatures/service-technicus-w-1280611"],
  naam: "TBI",
  rejectedReferenties: [],
};
const TENMONKS: BronSpec = {
  // Single fixture-backed URL — same reasoning as tbi above.
  abortOnDetailCall: 1,
  bronId: "00000000-0000-4000-8000-00000000000d",
  bronSlug: "tenmonks",
  categorie: "intermediair",
  config: tenmonksConfig,
  discoveredReferenties: ["opdrachten/34350/data-analist"],
  fixtureReferenties: ["opdrachten/34350/data-analist"],
  naam: "TenMonks",
  rejectedReferenties: [],
};
const JOBBIRD: BronSpec = {
  abortOnDetailCall: 2,
  ...
  discoveredReferenties: [
    "nl/vacature/25796307-freelance-inkoper-sociaal-domein-zzp",
    "nl/vacature/25849909-freelance-business-controller-zzp",
    "nl/vacature/25852520-freelance-adviseur-kcc-zzp",
  ],
  fixtureReferenties: [
    "nl/vacature/25796307-freelance-inkoper-sociaal-domein-zzp",
    "nl/vacature/25849909-freelance-business-controller-zzp",
...
describe.serial(
  "L3e JSON-LD cohort on the durable ingest path (CTP-641)",
  () => {
    let available = false;
    ...
    const registerSpecs = (spec: BronSpec): void => {
      ...
      it(`a changed ${spec.bronSlug} detail payload produces a new observation, a new aanvraag_versie and an updated curated row`, async () => {
        ...
        const mutatedTitle = `${targetBefore.titel} (CTP-641 gewijzigd)`;
        const { job, takes } = await driveJob(spec, {
          connector: () =>
