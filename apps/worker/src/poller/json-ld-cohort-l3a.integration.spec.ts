...
/**
 * CTP-637 L3a JSON-LD-cohort proof: ASML, BAM, Eneco, Heijmans and NS on the
 ...
 * full sitemaps/listings (bam 328 URLs, heijmans 563, eneco 142, ns 10,
 * asml 2), while detail coverage is the fixture-backed subset recorded per
 * bron (1–4 URLs). `scopedFixtureConnector` therefore filters the REAL parsed
 * listing to `config.detailFixtures` keys — every persisted payload is a real
 * recorded fixture, and whole-corpus enumeration itself is pinned per bron in
 * `packages/connectors/src/json-ld/durable-cohort-l3a.spec.ts` and the
 * per-source specs (e.g. `bam.spec.ts` asserts 328 discovered URLs).
 */
...
const previousEnv = {
  ASML_LIVE: process.env.ASML_LIVE,
  BAM_LIVE: process.env.BAM_LIVE,
  DATABASE_URL: process.env.DATABASE_URL,
  ENECO_LIVE: process.env.ENECO_LIVE,
  HEIJMANS_LIVE: process.env.HEIJMANS_LIVE,
  NS_LIVE: process.env.NS_LIVE,
...
process.env.RAW_OBJECT_STORE_PATH ??= `${process.env.TMPDIR ?? "/tmp"}/ji-jsonld-l3a-cohort-raw-${process.pid}`;
// All five connectors must stay in fixture mode — no live egress from tests.
delete process.env.ASML_LIVE;
delete process.env.BAM_LIVE;
delete process.env.ENECO_LIVE;
delete process.env.HEIJMANS_LIVE;
delete process.env.NS_LIVE;
...
interface BronSpec {
   ...
   * asml has a single fixture-backed URL (nothing can persist first — the
   * retake writes it fresh), and heijmans aborts on call 3 because call 1 is
   * the soft-404 fixture that is `rejected` before any persistence.
   */
...
const ASML: BronSpec = {
  ...
  discoveredReferenties: [
    "en/careers/find-your-job/senior-electrical-safety-expert-nominated-person--installatie-verantwoordelijke-euv-factory-j00333473",
  ],
  fixtureReferenties: [
    "en/careers/find-your-job/senior-electrical-safety-expert-nominated-person--installatie-verantwoordelijke-euv-factory-j00333473",
  ],
...
const BAM: BronSpec = {
  ...
  config: bamConfig,
  discoveredReferenties: [
    "nl/nl/job/24586/Werkvoorbereider-Elektrotechniek",
    "nl/nl/job/26209/Medewerker-Verkeersmaatregelen",
    "nl/nl/job/26832/Tendermanager-Bouw",
  ],
  fixtureReferenties: [
    "nl/nl/job/24586/Werkvoorbereider-Elektrotechniek",
    "nl/nl/job/26209/Medewerker-Verkeersmaatregelen",
...
const ENECO: BronSpec = {
  ...
  config: enecoConfig,
  discoveredReferenties: [
    "vacatures/ervaren-accountsupporter-3145",
    "vacatures/meewerkstage-dei-communicatie-employee-networks-2954",
    "vacatures/senior-trader-gas-2864",
  ],
  fixtureReferenties: [
    "vacatures/ervaren-accountsupporter-3145",
    "vacatures/meewerkstage-dei-communicatie-employee-networks-2954",
...
const NS: BronSpec = {
  ...
  config: nsConfig,
  discoveredReferenties: [
    "vacatures/conducteur-zwolle-zwolle-1331708",
    "vacatures/it-lead-ns-stations-utrecht-1316973",
    "vacatures/sap-run-manager-utrecht-utrecht-1320979",
  ],
  fixtureReferenties: [
    "vacatures/conducteur-zwolle-zwolle-1331708",
    "vacatures/it-lead-ns-stations-utrecht-1316973",
...
const SPECS: readonly BronSpec[] = [ASML, BAM, ENECO, HEIJMANS, NS];
...
describe.serial(
  "L3a JSON-LD cohort on the durable ingest path (CTP-637)",
  () => {
    let available = false;
    ...
    const registerSpecs = (spec: BronSpec): void => {
      it(`runs ${spec.bronSlug} end-to-end through the durable queue into curated aanvragen and the search outbox`, async () => {
        ...
        // Rejected items (heijmans' recorded soft-404) leave no rows at all.
      ...
      it(`a changed ${spec.bronSlug} detail payload produces a new observation, a new aanvraag_versie and an updated curated row`, async () => {
        ...
        const mutatedTitle = `${targetBefore.titel} (CTP-637 gewijzigd)`;
        const { job, takes } = await driveJob(spec, {
          connector: () =>
