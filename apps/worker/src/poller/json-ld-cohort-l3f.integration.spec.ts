...
/**
 * CTP-642 L3f JSON-LD-cohort proof: Randstad and Techniekwerkt on the
 ...
 * clients for detail reads, scripted whole-corpus listings for
 * retake/abort evidence.
 *
 * Both connectors share the single-page sitemap discovery contract at the
 * connector level: `discover()` returns every detail URL with
 * `hasMore: false` and an inert `checkpoint: {}`. A durable retake
 * re-enumerates the WHOLE corpus and re-fetches every detail page;
 * exactly-once is carried by the observation replay key
 * (scrapeRunId + bronReferentie + contentHash), not by page-cursor resume.
 * These specs assert that honestly.
 *
 * Corpus scoping, stated plainly: the committed listing fixtures are real
 * recordings (randstad's sitemap parses to 5 vacancy URLs, techniekwerkt's
 * gzip-inflated sitemap to 3), while detail coverage is the fixture-backed
 * subset recorded per bron. `scopedFixtureConnector` filters the REAL
 * parsed listing to `config.detailFixtures` keys — every persisted payload
 * is a real recorded fixture, and whole-corpus enumeration itself is
 * pinned per bron in
 * `packages/connectors/src/json-ld/durable-cohort-l3f.spec.ts`. Neither
 * bron has a recorded reject-fixture. Randstad DOES carry a third detail
 * fixture (`752363/teamleider`) whose URL is not in the recorded sitemap —
 * the scoped corpus cannot reach it, so randstad proves on the two
 * reachable detail-backed URLs and the third stays documented, not faked.
 *
 * The detail-read difference is pinned at connector level: both members
 * carry a `detailSynthesizer` — randstad's only surfaces `contactpersonen`
 * alongside an explicit JobPosting, while techniekwerkt's
 * `synthesizeJobPostingFromVike` is the ONLY JobPosting source (the
 * recorded pages have no ld+json JobPosting at all). Its fail-closed
 * behaviour when the vike structure is absent is pinned in the connector
 * spec; this file asserts the durable-path consequences (the payload that
 * DOES persist is the synthesized one).
 */
...
process.env.RAW_OBJECT_STORE_PATH ??= `${process.env.TMPDIR ?? "/tmp"}/ji-jsonld-l3f-cohort-raw-${process.pid}`;
// Both connectors must stay in fixture mode — no live egress from tests.
delete process.env.RANDSTAD_LIVE;
delete process.env.TECHNIEKWERKT_LIVE;
...
interface BronSpec {
   ...
   * one observation is persisted before the abort; both bronnen have three
   * fixture-backed URLs.
   */
...
const RANDSTAD: BronSpec = {
  ...
  discoveredReferenties: [
    "vacatures/741140/vrachtwagenchauffeur-allround",
    "vacatures/749250/operator",
  ],
  fixtureReferenties: [
    "vacatures/741140/vrachtwagenchauffeur-allround",
    "vacatures/749250/operator",
...
const TECHNIEKWERKT: BronSpec = {
  ...
  bronSlug: "techniekwerkt",
  categorie: "intermediair",
  config: techniekwerktConfig,
  discoveredReferenties: [
    "nl/vacature/leerling-monteur-werktuigbouwkunde-unica-rotterdam-973440",
    "nl/vacature/monteur-elektrotechniek-unica-eindhoven-973508",
    "nl/vacature/pcs-7-software-engineer-unica-zwolle-973447",
  ],
  fixtureReferenties: [
    "nl/vacature/leerling-monteur-werktuigbouwkunde-unica-rotterdam-973440",
    "nl/vacature/monteur-elektrotechniek-unica-eindhoven-973508",
...
const SPECS: readonly BronSpec[] = [RANDSTAD, TECHNIEKWERKT];
...
describe.serial(
  "L3f JSON-LD cohort on the durable ingest path (CTP-642)",
  () => {
    let available = false;
    ...
    const registerSpecs = (spec: BronSpec): void => {
      ...
      it(`a changed ${spec.bronSlug} detail payload produces a new observation, a new aanvraag_versie and an updated curated row`, async () => {
        ...
        const mutatedTitle = `${targetBefore.titel} (CTP-642 gewijzigd)`;
        const { job, takes } = await driveJob(spec, {
          connector: () =>
