...
import { createInhuurdeskEffectClient } from "./client-effect";
...
describe("inhuurdesk Effect read adapter", () => {
  it("fixture listing matches native Promise contract", async () => {
    const client = createInhuurdeskEffectClient({ liveEnabled: false });
    const listing = await client.fetchListing(1);
    expect(listing.data.length).toBeGreaterThan(0);
  });

  it("maps 401 through shared runtime", async () => {
    const client = createInhuurdeskEffectClient({
      fetchImpl: () => Promise.resolve(new Response("err", { status: 401 })),
      liveEnabled: true,
    });
    await expect(client.fetchListing(1)).rejects.toBeInstanceOf(AuthFault);
  });
  ...
  it("maps 5xx through shared runtime", async () => {
    ...
    await expect(client.fetchListing(1)).rejects.toBeInstanceOf(Server5xxFault);
  });
  ...
  it("honors AbortSignal during HTTP", async () => {
    ...
    const pending = client.fetchListing(1);
    await Bun.sleep(10);
    controller.abort();
    try {
      await pending;
