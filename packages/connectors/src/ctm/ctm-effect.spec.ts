...
import { createCtmEffectClient } from "./client-effect";
...
describe("ctm Effect read adapter", () => {
  it("fixture listing matches native Promise contract", async () => {
    const client = createCtmEffectClient({ liveEnabled: false });
  ...
  it("maps 401 through shared runtime", async () => {
    const client = createCtmEffectClient({
