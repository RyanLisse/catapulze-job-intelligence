...
import { createFlinterEffectClient } from "./client-effect";
...
describe("flinter Effect read adapter", () => {
  it("fixture listing matches native Promise contract", async () => {
    const client = createFlinterEffectClient({ liveEnabled: false });
    const listing = await client.fetchListing();
    expect(listing.length).toBeGreaterThan(0);
    const detail = await client.fetchDetailHtml("vergunningverlener-agrarisch");
    expect(detail.length).toBeGreaterThan(0);
  });

  it("maps 401 through shared runtime", async () => {
    const client = createFlinterEffectClient({
