...
import { createHarveyNashEffectClient } from "./client-effect";
...
describe("harveynash Effect read adapter", () => {
  it("fixture listing matches native Promise contract", async () => {
    const client = createHarveyNashEffectClient({ liveEnabled: false });
    ...
    expect(detail.facts).toBeTruthy();
  });

  it("maps 401 through shared runtime", async () => {
    const client = createHarveyNashEffectClient({
