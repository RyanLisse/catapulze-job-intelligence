...
/**
 * CTP-630: the JSON-LD cohort's resume contract differs from the feed
 * cohort's (CTP-629) in one honest way — there IS no page cursor.
 * `discovery.kind` is "sitemap" (bluetrail, pro-act) or "listing" (hero), so
 * `discover()` always returns the whole corpus with `hasMore: false` and an
 * empty `checkpoint: {}`. A durable retake therefore re-enumerates EVERY
...
const detailPayload = (url: string): JsonLdDetailPayload => ({
  jobPosting: {
    "@type": "JobPosting",
    hiringOrganization: { "@type": "Organization", name: "Broker" },
    title: `Opdracht ${url}`,
  },
  labelBlock: {},
  url,
...
const BLUETRAIL: BronSpec = {
  ...
  label: "bluetrail",
  urlFor: (slug) => `https://www.bluetrail.nl/opdrachten/Interim/${slug}/`,
};

const HERO: BronSpec = {
  config: heroConfig,
  label: "hero",
  urlFor: (slug) => `https://hero.eu/interim-opdrachten/${slug}`,
};

const PRO_ACT: BronSpec = {
  config: proActConfig,
  label: "pro-act",
  urlFor: (slug) => `https://pro-act.nl/vacatures/${slug}/`,
};
...
describe.each([
  ...
  ["hero", HERO],
  ["pro-act", PRO_ACT],
])("%s durable-cohort resume contract (CTP-630)", (_label, spec) => {
  ...
  it("an abort mid-item closes the run `failed` (persistence abort is never benign — CTP-490), with no committed checkpoint", async () => {
    ...
    // apps/worker/src/poller/json-ld-cohort.integration.spec.ts. The
