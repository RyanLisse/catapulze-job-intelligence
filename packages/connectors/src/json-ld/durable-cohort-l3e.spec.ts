...
/**
 * CTP-641: the L3e JSON-LD cohort (ProRail, Rijkswaterstaat, TBI, TenMonks,
 * Jobbird) shares the CTP-630 resume contract — at the CONNECTOR level there
 * IS no page cursor. `discover()` always returns the whole corpus with
 * `hasMore: false` and an inert `checkpoint: {}` for every discovery kind in
 * this cohort: `sitemap` (rijkswaterstaat, tbi, tenmonks), `listing`
 * (jobbird) and `json-listing` (prorail). ProRail's page walking lives inside
 * `client.fetchListing` (`discovery.pagination` fans out page 2..N before the
 * connector ever sees the list), and Jobbird deliberately covers listing page
 * one only — so a durable retake still re-enumerates EVERY detail URL and
...
const PRORAIL: BronSpec = {
  ...
  label: "prorail",
  urlFor: (slug) => `https://www.werkenbijprorail.nl/vacatures/functie/${slug}`,
};
...
const RIJKSWATERSTAAT: BronSpec = {
  ...
  urlFor: (slug) =>
    `https://werkenbij.rijkswaterstaat.nl/vacatures/${slug}/1300000`,
};

const TBI: BronSpec = {
  config: tbiConfig,
  label: "tbi",
  urlFor: (slug) => `https://werkenbij.tbi.nl/vacatures/${slug}-1280611`,
};

const TENMONKS: BronSpec = {
  config: tenmonksConfig,
  label: "tenmonks",
  urlFor: (slug) => `https://tenmonks.nl/opdrachten/34350/${slug}/`,
};

const JOBBIRD: BronSpec = {
  config: jobbirdConfig,
  label: "jobbird",
  urlFor: (slug) => `https://www.jobbird.com/nl/vacature/25800000-${slug}`,
};
...
describe.each([
  ...
  ["tenmonks", TENMONKS],
  ["jobbird", JOBBIRD],
])("%s durable-cohort resume contract (CTP-641)", (_label, spec) => {
  ...
  it("an abort mid-item closes the run `failed` (persistence abort is never benign — CTP-490), with no committed checkpoint", async () => {
    ...
    // apps/worker/src/poller/json-ld-cohort-l3e.integration.spec.ts. The
