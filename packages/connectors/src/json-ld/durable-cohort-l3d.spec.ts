...
/**
 * CTP-640: Hays is the L3d cohort's JSON-LD bron and shares the CTP-630
 * resume contract the L3a/L3b cohorts already proved — there IS no page
 * cursor. `discovery.kind` is "listing": one static search page
 * (`/vacatures-zoeken`) whose `/vacature-details/...` links are the whole
 * corpus, so `discover()` returns everything with `hasMore: false` and an
 * inert `checkpoint: {}`. A durable retake therefore re-enumerates EVERY
 * detail URL and re-fetches every detail page; exactly-once comes from the
 * observation replay key (scrapeRunId + bronReferentie + contentHash), not
 * from resuming mid-listing. Unlike the L3d feed siblings — and unlike
 * Haert — `knownHashes` is NOT forwarded by the hays source definition
 * (`listingHashCoversDetail: false`, and the listing card carries no
 * detail fields), so nothing short-circuits the detail reads.
 *
 * This file exists next to `durable-cohort-l3a.spec.ts`/
 * `durable-cohort-l3b.spec.ts`/`durable-cohort-l3c.spec.ts` rather than
 * inside any of them: L3d is a mixed cohort tracked as one deliverable,
 * and Hays is its only JSON-LD member — a separate file keeps the cohort
 * boundary readable.
 */
...
const BRON_ID = "bron-hays-l3d";

const detailPayload = (url: string): JsonLdDetailPayload => ({
  jobPosting: {
    "@type": "JobPosting",
    hiringOrganization: { "@type": "Organization", name: "Hays" },
    title: `Vacature ${url}`,
  },
  labelBlock: {},
  url,
...
const urlFor = (slug: string): string =>
  `https://www.hays.nl/vacature-details/${slug}-99999`;
...
/**
 * A single-page "listing page" served from `corpus` at call time, so mutating
 * the array between attempts reproduces a listing page that shifted while the
 ...
 * one detail read; `listingFailuresLeft` scripts a listing outage.
 */
const scriptedClient = (options: {
  ...
  fetchListing: () => {
    ...
    if (
      ...
      return Promise.reject(new Error("scripted Hays listing outage"));
    }
...
describe("hays durable-cohort resume contract (CTP-640)", () => {
  const slugs = ["alpha-1", "beta-2", "gamma-3"];
  const refs = slugs.map(referentieFor);

  it("enumerates the whole filtered listing page in one discover call — hasMore: false, an inert empty checkpoint, never truncated", async () => {
  ...
  it("a durable retake of a killed run re-enumerates the whole listing; the replay key absorbs persisted items", async () => {
    const scrapeRunId = "run-hays-l3d-resume";
    const corpus = corpusOf(slugs);
    const calls: ClientCalls = { detail: [], listing: 0 };
  ...
  it("picks up a head insert on the retake itself — there is no read-pages blind spot", async () => {
    const scrapeRunId = "run-hays-l3d-insert";
    const corpus = corpusOf(slugs);
    const calls: ClientCalls = { detail: [], listing: 0 };
    const client = scriptedClient({
      calls,
  ...
  it("a failed listing read closes the run `failed` (DISCOVER_FAILED) with no committed checkpoint", async () => {
    const scrapeRunId = "run-hays-l3d-discoverfail";
    const calls: ClientCalls = { detail: [], listing: 0 };
    const client = scriptedClient({
      calls,
  ...
  it("an abort mid-item closes the run `failed` (RAW_STORE_WRITE_FAILED — persistence abort is never benign, CTP-490), with no committed checkpoint", async () => {
    const scrapeRunId = "run-hays-l3d-abort";
    const corpus = corpusOf(slugs);
    const calls: ClientCalls = { detail: [], listing: 0 };
