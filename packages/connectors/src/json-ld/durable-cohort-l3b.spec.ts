...
import { volkerwesselsConfig } from "./configs/volkerwessels";
...
/**
 * CTP-638: the L3b JSON-LD cohort (DataJobs.nl, Unica, Vattenfall,
 * VolkerWessels, Werken voor Nederland) shares the CTP-630 resume contract —
 * there IS no page cursor. `discovery.kind` is "sitemap" for all five, so
 ...
 * from resuming mid-listing. These specs pin that contract per bron.
 */
 ...
 * array between attempts reproduces a sitemap that shifted while the run was
...
const DATAJOBS: BronSpec = {
  ...
  label: "datajobs",
  urlFor: (slug) => `https://www.datajobs.nl/vacatures/${slug}`,
};
...
const UNICA: BronSpec = {
  ...
  label: "unica",
  urlFor: (slug) =>
    `https://www.werkenbijunica.nl/vacatures/${slug}-aqk99zz0yy8x`,
};
...
const VATTENFALL: BronSpec = {
  ...
  label: "vattenfall",
  urlFor: (slug) =>
    `https://careers.vattenfall.com/global/job/${slug}-in-amsterdam-jid-99000`,
};
...
const VOLKERWESSELS: BronSpec = {
  ...
  label: "volkerwessels",
  urlFor: (slug) =>
    `https://www.werkenbijvolkerwessels.nl/vacature/99000/${slug}`,
};

const WERKEN_VOOR_NEDERLAND: BronSpec = {
  config: werkenVoorNederlandConfig,
  label: "werken-voor-nederland",
  urlFor: (slug) =>
    `https://www.werkenvoornederland.nl/vacatures/${slug}-2026-9900`,
};
...
describe.each([
  ...
  ["werken-voor-nederland", WERKEN_VOOR_NEDERLAND],
])("%s durable-cohort resume contract (CTP-638)", (_label, spec) => {
  ...
  it("an abort mid-item closes the run `failed` (persistence abort is never benign — CTP-490), with no committed checkpoint", async () => {
    ...
    // apps/worker/src/poller/json-ld-cohort-l3b.integration.spec.ts. The
